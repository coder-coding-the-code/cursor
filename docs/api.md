...
| 提示词安全 | [NVIDIA NeMo Guardrails](https://github.com/NVIDIA-NeMo/Guardrails) `hf_classifier` local |
| 补充签名 | Vigil YARA 规则 + VirusTotal `yara-python` |
| 本体 | `rdflib` 解析 SPARQL Update / JSON-LD |
| 隐藏载荷 | `detect-secrets` |

NVIDIA NeMo Guardrails 0.24.0 仍在维护。默认模型 `protectai/deberta-v3-base-prompt-injection-v2`，阈值 `0.92`，拦截标签 `INJECTION`（`SEMANTIC_FW_NEMO_MODEL` / `SEMANTIC_FW_NEMO_THRESHOLD` 可覆盖）。短于 16 字的隐藏片段不送进分类器。
...
| `stages[]` | canonicalize / nemo_guardrails / detectors / session_chain / policy |
